...
const { readFile, writeFile, createFolder } = require("../fs");
...
async function init() {
      ...
      const { url } = await inquirer.prompt([
        {
          ...
          validate: v => {
            ...
            if (v.indexOf("/") > -1) {
              return (
                "The URL shold not contain sub-folders, all pages are generated on root level [" +
                toSlug(pageName) +
                "]."
              );
            }
  ...
  if (pages.length !== 0) {
    createFolder(__dirname + "/../../src/Pages");
    pages.map(page => {
    ...
    writeFile(
      __dirname + "/../../src/Pages/index.json",
      JSON.stringify(
        pages.map(item => {
          return {
            name: item.pageName,
  ...
  if (formSteps.length !== 0) {
    createFolder(__dirname + "/../../src/Form");
    writeFile(
      __dirname + "/../../src/Form/index.json",
      JSON.stringify(
        formSteps.reduce(
          (accumulator, item, index) => {
            return {
              ...accumulator,
              steps: accumulator.steps.concat({
                step: index + 1,
                name: item.stepName
              })
            };
    ...
    formSteps.map((step, index) => {
      if (step.isTemplate === true) {
        const content = readFile(
          __dirname + "/components/templates/formStep.js"
        )
          .replace("/* notConnected */", "default")
          .replace(new RegExp(/\/\* (.*) \*\//, "g"), "");

        createFolder(__dirname + "/../../src/Form/Step" + (index + 1));
        writeFile(
        ...
        const content = readFile(
          ...
          .replace(
            ...
            unique(
              step.stepContent
                ...
                .reduce(
                  (accumulator, item) => accumulator.concat(item.import),
                  []
                )
            ).join("\n")
          )
          .replace(
            ...
            step.stepContent
              ...
              .map(
                content =>
                  `${content.formikKey}: data.${content.formikKey} || ''`
              )
          )
          .replace(
            "/* content */",
